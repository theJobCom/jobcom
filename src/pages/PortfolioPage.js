...
const PortfolioPage = () => {
  ...
  useEffect(() => {
    const q = query(collection(db, "Contact"), where("createdBy", "==", doc(db, "User", id)));
    const unsubScribe = onSnapshot(q, (snapShot) => {
      let dataArr = []
      snapShot.docs.forEach((doc) => {
        dataArr.push({ ...doc.data(), id: doc.id })
  ...
  useEffect(() => {
    const q = query(collection(db, "Resumes"), where("createdBy", "==", doc(db, "User", id)));
    const unsubScribe = onSnapshot(q, (snapShot) => {
      let dataArr = []
      snapShot.docs.forEach((doc) => {
        dataArr.push({...doc.data(), id: doc.id})
  ...
  useEffect(() => {
    const q = query(collection(db, "Avatars"), where("createdBy", "==", doc(db, "User", id)));
    const unsubScribe = onSnapshot(q, (snapShot) => {
      let dataArr = []
      snapShot.docs.forEach((doc) => {
        dataArr.push({ ...doc.data(), id: doc.id })
  ...
  useEffect(() => {
    const q = query(collection(db, "Achievements"), where("createdBy", "==", doc(db, "User", id)));
    const unsubScribe = onSnapshot(q, (snapShot) => {
      let dataArr = []
      snapShot.docs.forEach((doc) => {
        dataArr.push({...doc.data(), id: doc.id})
